...
inline fun VTableBuilder.flowProp(
...
data class FlowPropertyVTableItem(
    val name: PropertyName,
    var signature: Signature? = null,
    var getter: PropertyGetCallback? = null,
    val flags: Flags = Flags()
) : VTableItem {
    inline fun <reified T : Any> withGetter(crossinline callback: () -> Flow<T>): FlowPropertyVTableItem =
        apply {
            if (signature == null) {
                signature = Signature(signatureOf<T>().value)
            }
